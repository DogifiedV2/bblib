...
public class BBEntityRenderer<T extends Entity & BBAnimatable> extends EntityRenderer<T> {
    ...
    private void renderCubeFree(BBModel<T> renderModel, T entity, ModelData modelData, CubeData cube, ResourceLocation modelId,
    ...
    private void applyBillboardFacing(T entity, PoseStack poseStack, BillboardData billboard) {
        ...
        Matrix4f inversePose = poseStack.last().pose().copy();
        if (!inversePose.invert()) {
            return;
        }
        ...
        localCamera.transform(inversePose);
